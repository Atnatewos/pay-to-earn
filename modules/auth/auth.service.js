        ...
        try {
            ...
            const result = await client.query(
                `INSERT INTO users (phone, full_name, password_hash, referral_code, referred_by, parent_id, active_package, package_expiry)
                 VALUES ($1, $2, $3, $4, $5, $6, 'Intern', CURRENT_DATE + INTERVAL '3 days')
                 RETURNING id`,
            ...
            if (parentId) {
                await this.buildTreeRelations(client, userId, parentId);
            }

            // Create daily tasks
            await client.query(
                'INSERT INTO daily_tasks (user_id, task_date, tasks_allocated) VALUES ($1, CURRENT_DATE, $2)',
                [userId, INTERN.TASKS_PER_DAY]
            );
