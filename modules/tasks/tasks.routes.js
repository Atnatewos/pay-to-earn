...
router.get('/today', authenticateUser, async (req, res, next) => {
    try { const task = await TasksService.getTodayTask(req.user.id); return Response.success(res, task); }
    catch (error) { next(error); }
});

router.post('/complete', authenticateUser, taskLimiter, async (req, res, next) => {
    try { const result = await TasksService.completeTask(req.user.id); return Response.success(res, result, 'Task completed'); }
    catch (error) { next(error); }
});
...
router.get('/earnings', authenticateUser, async (req, res, next) => {
    try { const summary = await TasksService.getEarningsSummary(req.user.id); return Response.success(res, summary); }
    catch (error) { next(error); }
});
