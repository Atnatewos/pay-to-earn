...
router.get('/overview', authenticateUser, async (req, res, next) => {
    try { const overview = await TeamService.getTeamOverview(req.user.id); return Response.success(res, overview); }
...
router.get('/level/:level', authenticateUser, async (req, res, next) => {
    try {
        ...
        if (level < 1 || level > 3) return Response.error(res, 'Level must be 1, 2, or 3', 400);
...
router.get('/referral-link', authenticateUser, async (req, res, next) => {
    try { const result = await TeamService.getReferralLink(req.user.id); return Response.success(res, result); }
    catch (error) { next(error); }
});
