...
class HomePage {
  ...
  async render() {
    ...
    setTimeout(() => HomePage.calculateDaysRemaining(), 300);
  }
  ...
  async loadStickyAlerts() {
    try {
      ...
      var response = await fetch(apiUrl + '/alerts/sticky', {
        headers: { 'Authorization': 'Bearer ' + token }
      });
      var result = await response.json();

      if (result.success && result.data && result.data.length > 0) {
        var alertsContainer = document.getElementById('stickyAlerts');
        if (!alertsContainer) return;

        result.data.forEach(function(alert) {
          var alertEl = document.createElement('div');
          var bgColor = alert.color === 'color-danger' ? 'var(--color-danger-bg)'
            : alert.color === 'color-warning' ? 'var(--color-warning-bg)'
            : 'var(--color-info-bg)';
          var borderColor = alert.color === 'color-danger' ? 'var(--color-danger)'
            : alert.color === 'color-warning' ? 'var(--color-warning)'
            : 'var(--color-info)';

          alertEl.style.cssText = 'background:' + bgColor + ';border-left:4px solid ' + borderColor + ';padding:12px 16px;display:flex;align-items:center;gap:12px;margin-bottom:8px;border-radius:var(--radius-lg);font-size:var(--font-sm);';
          alertEl.innerHTML = '<span style="font-size:24px;">' + alert.icon + '</span><div style="flex:1;"><strong>' + alert.title + '</strong><div style="color:var(--color-text-secondary);">' + alert.message + '</div></div><button onclick="this.parentElement.remove();fetch(\'' + apiUrl + '/alerts/' + alert.id + '/dismiss\',{method:\'POST\',headers:{\'Authorization\':\'Bearer ' + token + '\'}});" style="font-size:20px;cursor:pointer;opacity:0.5;padding:4px 8px;">×</button>';
  ...
  async loadData() {
    try {
      var profileData = await API.get('/auth/profile');
      var earningsData = await API.get('/tasks/earnings');
      var taskData = await API.get('/tasks/today');

      var user = profileData.data;
      var earn = earningsData.data;
      var todayTask = taskData.data;
      ...
      document.getElementById('homeContent').innerHTML = `
          ...
          ${user.active_package ? `
            <div class="package-expiry-info mt-3">
              <div class="flex items-center gap-2 mb-2">
                ...
                ${user.manager_rank ? '<span class="badge ml-1" style="background:linear-gradient(135deg,#FFD700,#FFA500);color:#000;">🏆 ' + user.manager_rank + '</span>' : ''}
              </div>
              ${user.package_expiry ? `
        ...
        <div class="stats-row">
          <div class="stat-card-home animate-fadeInUp stagger-1" onclick="router.navigate('/tasks')">
            ...
            <div class="stat-card-sub">${this.formatETB(todayTask ? todayTask.earned || 0 : 0)} ETB earned</div>
          </div>
        ...
        <div class="summary-card animate-fadeInUp stagger-4">
          ...
          <div class="summary-card-body">
            <div class="summary-row"><span class="summary-label">Yesterday</span><span class="summary-value">${this.formatETB(earn.yesterdayEarnings)} ETB</span></div>
            <div class="summary-row"><span class="summary-label">This Week</span><span class="summary-value">${this.formatETB(earn.weekEarnings)} ETB</span></div>
            <div class="summary-row"><span class="summary-label">This Month</span><span class="summary-value">${this.formatETB(earn.monthEarnings)} ETB</span></div>
            <div class="summary-row"><span class="summary-label">Total Earned</span><span class="summary-value highlight">${this.formatETB(earn.totalEarned)} ETB</span></div>
          </div>
        </div>
        ...
        <div class="summary-card animate-fadeInUp stagger-5">
          ...
          <div class="summary-card-body">
            <div class="summary-row"><span class="summary-label">Referral Commissions</span><span class="summary-value highlight">${this.formatETB(earn.referralCommissions)} ETB</span></div>
            <div class="summary-row"><span class="summary-label">Task Commissions</span><span class="summary-value highlight">${this.formatETB(earn.taskCommissions)} ETB</span></div>
            <div class="summary-row"><span class="summary-label">Total Deposited</span><span class="summary-value">${this.formatETB(earn.totalDeposited)} ETB</span></div>
          </div>
        </div>
      `;
    } catch (error) {
  ...
  static calculateDaysRemaining() {
    ...
    if (!expiryEl) return;
    var expiryDate = new Date(expiryEl.dataset.expiry);
    var now = new Date();
    var diffDays = Math.ceil((expiryDate - now) / (1000 * 60 * 60 * 24));
    var daysEl = document.getElementById('daysRemaining');
    var progressEl = document.getElementById('expiryProgress');
    if (daysEl) {
