...
/**
 * User Profile Page
 * Shows user info with copy buttons on all important fields
 * Manager rank badge displayed when assigned
 */
class ProfilePage {
  ...
  async render() {
    ...
    this.container.innerHTML = '<div class="page"><div id="profileContent"></div></div>';
    router.reinjectNavigation();
    await this.loadProfile();
  }

  async loadProfile() {
    Loader.auto('profileContent');
      ...
      document.getElementById('profileContent').innerHTML = `
        <div class="profile-header card card-gradient text-center mb-4" onclick="ProfilePage.showEditProfile()" style="cursor:pointer;">
          ...
          <p class="text-sm text-secondary">
            ${user.phone}
            <button class="btn btn-ghost btn-sm" onclick="event.stopPropagation();navigator.clipboard.writeText('${user.phone}');Toast.show('Phone copied!')" title="Copy phone">📋</button>
          </p>
          <p class="text-sm text-secondary">
            Referral: <strong>${user.referral_code}</strong>
            <button class="btn btn-ghost btn-sm" onclick="event.stopPropagation();navigator.clipboard.writeText('${user.referral_code}');Toast.show('Code copied!')" title="Copy code">📋</button>
          </p>
          <div class="mt-2">
  ...
  static async showEditProfile() {
    ...
    document.getElementById('editProfileForm').addEventListener('submit', async (e) => {
      ...
      try {
        await fetch(APP_CONFIG.apiUrl + '/auth/profile', {
          method: 'PUT',
